<template>
  <div
    class="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#E8F5EC] via-white to-slate-50 px-4"
  >
    <div v-if="isLoading" class="flex items-center justify-center">
      <div
        class="h-8 w-8 animate-spin rounded-full border-4 border-[#C8EDD4] border-t-[#4CAF6F]"
      />
    </div>
    ...
    <div v-else class="w-full max-w-sm">
      ...
      <div class="mb-8 text-center">
        <div
          class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#4CAF6F] shadow-lg"
        >
          <svg
      ...
      <!-- カード -->
      <div
        class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xl ring-1 ring-slate-900/5"
      >
        <!-- Google ログイン -->
        <button
          type="button"
          ...
          <div class="space-y-4">
            <div>
              ...
              <input
                ...
                class="mt-1.5 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm placeholder-slate-400 shadow-sm transition-all duration-150 focus:border-[#B8E0C4] focus:ring-2 focus:ring-[#C8EDD4] focus:outline-none disabled:bg-slate-50"
              />
            </div>
            ...
            <button
              type="submit"
              :disabled="submitting"
              class="flex w-full items-center justify-center gap-2 rounded-xl bg-[#4CAF6F] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-all duration-150 hover:bg-[#388E54] hover:shadow-md focus:ring-2 focus:ring-[#4CAF6F] focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
            >
              <svg
                v-if="submitting"
