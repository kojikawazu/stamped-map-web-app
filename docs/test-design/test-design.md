...
- [テスト構成の全体像](#テスト構成の全体像)
  - [ファイル配置](#ファイル配置)
- [1. ユニットテスト](#1-ユニットテスト)
  - [1-0. `lib/map-utils`（lib/map-utils.ts）](#1-0-libmap-utilslibmap-utilsts)
    - [`escapeHtml`](#escapehtml)
    - [`buildCategoryMap`](#buildcategorymap)
    - [`markersToGeoJSON`](#markerstogeojson)
  - [1-1. `useSpotFilter`（composables/useSpotFilter.ts）](#1-1-usespotfiltercomposablesusespotfilterts)
    - [正常系](#正常系)
    - [準正常系](#準正常系)
  - [1-2. `useApiClient`（composables/useApiClient.ts）](#1-2-useapiclientcomposablesuseapiclientts)
    - [正常系](#正常系-1)
    - [準正常系](#準正常系-1)
- [2. 結合テスト（Server Routes）](#2-結合テストserver-routes)
  - [共通前提](#共通前提)
  - [2-1. `GET /api/spots`](#2-1-get-apispots)
    - [正常系](#正常系-2)
    - [準正常系](#準正常系-2)
  - [2-2. `GET /api/spots/markers`](#2-2-get-apispotsmarkers)
    - [正常系](#正常系-3)
    - [準正常系](#準正常系-3)
  - [2-3. `POST /api/spots`](#2-3-post-apispots)
    - [正常系](#正常系-4)
    - [準正常系](#準正常系-4)
  - [2-4. `GET /api/spots/:id`](#2-4-get-apispotsid)
    - [正常系](#正常系-5)
    - [準正常系](#準正常系-5)
  - [2-5. `PUT /api/spots/:id` / `DELETE /api/spots/:id`](#2-5-put-apispotsid--delete-apispotsid)
    - [正常系・準正常系](#正常系準正常系)
  - [2-6. Categories API](#2-6-categories-api)
- [3. E2Eテスト（Playwright）](#3-e2eテストplaywright)
  - [共通設定](#共通設定)
  - [helpers.ts の提供内容](#helpersts-の提供内容)
  - [3-1. スモークテスト（smoke.spec.ts）](#3-1-スモークテストsmokespects)
  - [3-2. 認証フロー（auth.spec.ts）](#3-2-認証フローauthspects)
    - [正常系](#正常系-6)
    - [準正常系](#準正常系-6)
  - [3-3. スポット一覧・フィルター（spots.spec.ts）](#3-3-スポット一覧フィルターspotsspects)
    - [正常系](#正常系-7)
    - [準正常系](#準正常系-7)
  - [3-4. 地図マーカー（spots.spec.ts に含める）](#3-4-地図マーカーspotsspects-に含める)
    - [正常系](#正常系-8)
- [4. GitHub Actions CI](#4-github-actions-ci)
  - [ワークフロー設計](#ワークフロー設計)
- [5. テスト追加に伴う package.json 変更](#5-テスト追加に伴う-packagejson-変更)
- [実装順序](#実装順序)
