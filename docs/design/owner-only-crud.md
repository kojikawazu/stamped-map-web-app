...
- [実装設計](#実装設計)
  - [環境変数](#環境変数)
  - [`server/utils/auth.ts` への追加](#serverutilsauthts-への追加)
  - [各エンドポイントの変更](#各エンドポイントの変更)
  - [フロントエンド側の対応](#フロントエンド側の対応)
    - [ボタン表示制御](#ボタン表示制御)
- [テスト設計](#テスト設計)
  - [ユニットテスト（`server/utils/auth.ts`）](#ユニットテストserverutilsauthts)
  - [APIエンドポイントテスト](#apiエンドポイントテスト)
- [`.env.example` への追記](#envexample-への追記)
